...
export const DrivingProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    const [locationState, setLocationState] = useLocalStorageBoolean(
        "Réside à moins de  50 km du havre ?",
        null
    );
    const [hasPermit, setHasPermit] = useLocalStorageBoolean("Permis de conduire", null);
    const [supervisedDriving, setSupervisedDriving] = useLocalStorageBoolean(
        "Conduite accompagnée ou supervisée",
        null
    );
    const [isAccompanist, setIsAccompanist] = useLocalStorageBoolean("Accompagnateur", null);
    ...
    return <DrivingContext.Provider value={contextValue}>{children}</DrivingContext.Provider>;
};
