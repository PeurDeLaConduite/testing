...
export const useSlides = (
    ...
    }, [setParam, setStopTimerButton, currentSlide, setCurrentSlide, sliderContentLength]);

    const prevSlide = useCallback(() => {
        sessionStorage.removeItem("slideRef");
        const prevValue = (currentSlide - 1 + sliderContentLength) % sliderContentLength;
        setParam("slideRef", String(prevValue));
        setStopTimerButton(true);
        setCurrentSlide(prevValue);
    }, [currentSlide, setParam, setStopTimerButton, setCurrentSlide, sliderContentLength]);
