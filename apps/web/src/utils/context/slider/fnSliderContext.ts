...
export const sessionSlideRef = (setCurrentSlide: React.Dispatch<React.SetStateAction<number>>) => {
...
export const manageAutoSlide = (
    ...
    const savedSlideRef = sessionStorage.getItem("slideRef");
    if (stopTimerButton || (savedSlideRef !== null && !isNaN(Number(savedSlideRef)))) {
        return () => {};
    }
    const slideInterval = startSlideInterval(sliderContentLength, intervalTime, setCurrentSlide);
