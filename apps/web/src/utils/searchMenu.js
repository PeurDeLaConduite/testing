export default function searchQuery(jsonData, query) {
    ...
    const ignoredKeys = ["id", "AnchorId", "class", "svg", "path", "link", "alt", "icon"];
    ...
    function searchInItems(items, basePath = "") {
        ...
        items.forEach((item) => {
            ...
            const currentPath = item.path
                ? item.path
                : `${basePath}${item.AnchorId ? `/${item.AnchorId}` : ""}`.replace(/\/+/g, "/");
                ...
                if (
                    ...
                    value.forEach((arrayItem) => {
                        if (
                            ...
                            arrayItem.toLowerCase().includes(query.toLowerCase())
                        ) {
